...
class ImageToPDFConverter:
    ...
    def initialize_ui(self):
        title_label = tk.Label(self.root, text ="ImageToPDFConverter", font=("Helvetica", 16,"bold"))
    ...
    def date_selected_image_listbox(self):
        self.selected_images_listbox.delete(0, tk.END)
        
        for image_path in self.image_paths:
            _, image_path = os.path.split(image_path)
            self.date_selected_image_listbox.insert(tk.END, image_path)
    ...
    def convert_images_to_pdf(self):
        ...
        for image_path in self.image_paths:
            ...
            new_heigt = img.height * scale_factor
            x_centered = (612 - new_width) /2
            y_centered = (782 - new_heigt) /2

            pdf.setFillcolorRGB(255, 255, 255)
            pdf.rect(0, 0, 612, 792,fill=True)
            pdf.drawInlineImage(img, x_centered, width=new_width, heigt=new_heigt)
